exports.simplifyExpenses = (contributors, divisionMethod) => {
    ...
    if (divisionMethod === "equal") {
        ...
        transactions = separateEqual(contributors, totalExpense);
    ...
    for (const creditor of creditors) {
        while (creditor.owed !== 0) {
            for (let i = 0; i < debtors.length;) {
                ...
                if (x > 0) {
                    transactions.push({paidBy: debtor.name, paidTo: creditor.name, amount: debtor.owes});
                    creditor.owed = x;
                    debtor.owes = 0;
                    debtors.splice(i, 1); // remove debtor from array
                } else if (x < 0) {
                    transactions.push({paidBy: debtor.name, paidTo: creditor.name, amount: creditor.owed});
                    creditor.owed = 0;
                    debtor.owes = -x;
                    i++;
                } else {
